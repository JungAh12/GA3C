...
class ThreadDynamicAdjustment(Thread):
    ...
    def run(self):
        ...
        while not self.exit_flag:
            old_trainer_count, old_predictor_count, old_agent_count =                 self.trainer_count, self.predictor_count, self.agent_count
            self.random_walk()

            # If no change, do nothing
            if self.trainer_count == old_trainer_count                     and self.predictor_count == old_predictor_count                     and self.agent_count == old_agent_count:
                continue
            ...
            if cur_count < old_count:
                self.trainer_count, self.predictor_count, self.agent_count =                     old_trainer_count, old_predictor_count, old_agent_count
